	...
	*/
	...
	void executePipelined(){
		...
		pr MEM_WB = {.currcomand = str, .at_PC = -1, .PC_new = -1, .WB = &wbctr4, .MEM = &memctr4, .EX = &exctr4,
		 ...
		 bool is_beq_stall = false ; bool is_j_stall = false ; bool check = false;  bool is_stall = false ; 
        ...
        int op1 = 0 , op2 = 0 ;  int write_data = 0 ;
		string rs, rt, rd ; 
		...
		{	
			...
			if (ALU_MEM.at_PC >= 0 ){
                ...
                // cout << "ALU_MEM.RT " << ALU_MEM.RT << "MEM_WB.RD " << MEM_WB.RD << "\n" ; 
				...
				if ((ALU_MEM.MEM)->MemWrite) {
					...
					// cout << "wrote " << write_data << " at address " << (ALU_MEM.ALURESULT)/4<< "\n" ;
				} 
			...
			if (ID_ALU.at_PC >= 0){
				...
				if ((ID_ALU.EX)->OP == 0){
						...
						// cout << "added " << op1 << " and " << op2 << "to get " << ALU_MEM.ALURESULT<< "\n" ; 
				}
				else if ((ID_ALU.EX)->OP == 1){	
					ALU_MEM.ALURESULT = op1 - op2 ;
					// cout << "subtracted " << op2 << " from " << op1 << "to get " << ALU_MEM.ALURESULT<< "\n" ;
				}

				else if ((ID_ALU.EX)->OP == 2) {
					ALU_MEM.ALURESULT = op1 * op2;  
					// cout << "multiplying  " << "op1 " << "op2 " << ALU_MEM.ALURESULT << "\n" ; 
				}
